...
class ScannerOld():
	...
	'''
	This class searches devices connected
	Wacom Intuos PT M 2 Pen stylus  	id: 11	type: STYLUS    
	Wacom Intuos PT M 2 Finger touch	id: 12	type: TOUCH     
	Wacom Intuos PT M 2 Pad pad     	id: 13	type: PAD 
	For each device detected:
	- create model in memory
	- retrieve properties from device and store in memory
	Once the models are ready create/update the ui for each model
	'''

	def __init__(self, app, registry):
		self._app = app
		self._keepRunning = False
		self._registry = registry # DevicesRegistry()
		self._period = 1          # seconds
		self._iterations = 0;
	...
	def scan(self):
		...
		#self._keepRunning = False
		self._period = 5  # Run less frequently after the first 3 iterations @1 sec. 
		                  # TODO: implement algorighm to calculate the period 
		...
		for device in devices:
			self._registry.set_devices_status_checking()
			self._registry.register_device(device_set_name, device['id'], device['name'], device['type'])	

		# Devices still in STATUS_CHECKING have to be removed (not in the list)
		self._registry.remove_devices_on_checking(device_set_name)
